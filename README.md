# meikyu-rogue — 迷宮ローグ

ターン制ローグライク。シードから決定的に生成される 5 階層の迷宮を探索し、
最下層 B5 の秘宝「暁の勾玉」を持ち帰れば勝利。

**▶ プレイ: https://meikyu-rogue.vercel.app** ・ リポジトリ: https://github.com/twill3c/meikyu-rogue
・ [迷宮ローグの歩き方(遊び方ガイド)](https://claude.ai/code/artifact/cc33a7fa-c27a-4261-abb9-a8c0043cec99)
・ [迷宮ローグ設計図(アーキテクチャ)](https://claude.ai/code/artifact/6620befa-ac12-4085-a8ec-aa4054329cfc)

- **決定的**: 同一シードなら同一の迷宮・同一のリプレイ(乱数は GameState 内で持ち回り)
- **検証済み生成**: すべての階は「入口から階段・全アイテム・全敵へ到達可能」を
  フラッドフィルで検証してから出荷される
- **静的サイト**: サーバなし。`next build` の out/ だけで動く
- **スコア**: 敵撃破(種別ポイント)+ 階降下 +50 + 暁の勾玉 +200。敵は 5 種
  (スライム/こうもり=2 回行動/ゴブリン/オーガ/レイス=壁越し感知)が深度別に出現
- **ハイスコア**: 上位 10 件を localStorage に保存(サーバ通信なし)。
  ヘッダに best、決着画面に TOP5 と今回の順位を表示
- **効果音**: Web Audio 合成(音源ファイルなし・BGM なし)。**デフォルト OFF**。
  ヘッダのトグルまたは m キーで ON/OFF、設定は localStorage に保存

## 遊び方

| キー | 動作 |
|---|---|
| 矢印 / hjkl / wasd | 移動(敵に向かえば攻撃) |
| `.` | 待機(階段上では降りる) |
| q | 回復薬を飲む |
| m | 効果音 ON/OFF(デフォルト OFF) |
| r | 新しいシードで再挑戦 |

## 開発

```bash
npm install
npm run dev           # 開発サーバ
npm run verify:fast   # typecheck + lint + test
npm run verify        # 上記 + next build(完了条件)
```

開発規範は [AGENTS.md](AGENTS.md)、仕様は [SPEC.md](SPEC.md)、
テスト対応表は [TEST_SPEC.md](TEST_SPEC.md) を参照。
ループログは `logs/loops/` に JSONL で残る(loop-observability 規律)。
